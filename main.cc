...
int main()
{
    int Nx(64), Ny(128),k(0),kmax(1000);
    double dt(0.025),dx,dy,D, Lx(1), Ly(1), tmax(50.0),eps(0.0001), t(0.);
  ...
  for (int j(0); j<Ny; j++)
  {
    ...
    b=RHS(dt,Nx,Ny,u,t);
    u=GC(x0,b,eps,kmax,Nx,Ny,dt);double normL2_2D(vector<double> x, double dx, double dy);
    // for (int k(0); k<Nx*Ny; k++)
    //     {
    //       std::cout<< u[k]<<std :: endl;
    //     }
